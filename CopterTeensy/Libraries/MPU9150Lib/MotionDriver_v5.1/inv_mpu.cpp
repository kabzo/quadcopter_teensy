...
int mpu_init(struct int_param_s *int_param)
{
    ...
    if (rev) {
...
#ifdef DEBUG
            GCS._serial.print("Unsupported software product rev: "); GCS._serial.println(rev);
#endif
            return -4;
        }
    } else {
        ...
        if (!rev) {
#ifdef DEBUG
            GCS._serial.println("Product ID read as 0 indicates device is either incompatible or an MPU3050");
#endif
            return -6;
        } else if (rev == 4) {
#ifdef DEBUG
            GCS._serial.println("Half sensitivity part found.");
#endif
            st->chip_cfg.accel_half = 1;
        } else
            st->chip_cfg.accel_half = 0;
...
#ifdef AK89xx_SECONDARY
#ifdef DEBUG
    GCS._serial.println("Setting up compass");
#endif
    errCode = setup_compass();
    if (errCode != 0) {
#ifdef DEBUG
       GCS._serial.print("Setup compass failed: "); GCS._serial.println(errCode);
#endif
    }
...
#ifdef DEBUG
            GCS._serial.print("fimrware write failed: ");
            GCS._serial.println(errCode);
#endif
            return -3;
        }
...
#ifdef DEBUG
            GCS._serial.print("Firmware compare failed addr "); GCS._serial.println(ii);
            for (int i = 0; i < 10; i++) {
              GCS._serial.print(progBuffer[i]);
              GCS._serial.print(" ");
            }
            GCS._serial.println();
            for (int i = 0; i < 10; i++) {
              GCS._serial.print(cur[i]);
              GCS._serial.print(" ");
            }
            GCS._serial.println();
#endif
...
#ifdef DEBUG
    GCS._serial.println("Firmware loaded");
#endif
    return 0;
}
...
#ifdef DEBUG
        GCS._serial.println("Compass not found.");
#endif
        return -1;
    }
...
#ifdef DEBUG
    GCS._serial.print("Compass sens: "); GCS._serial.print(st->chip_cfg.mag_sens_adj[0]); GCS._serial.print(" ");
    GCS._serial.print(st->chip_cfg.mag_sens_adj[1]); GCS._serial.print(" ");
    GCS._serial.print(st->chip_cfg.mag_sens_adj[2]); GCS._serial.println();
#endif
