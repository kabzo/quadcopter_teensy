...
float Mav_Param::get_param_value_id(uint8_t id, enum var_type &type) {
	Mav_Param *param = get_param_id(id, type);
	return param->cast_to_float_mav(type);
}

float Mav_Param::get_param_value_id_name(	uint8_t &id,
																					enum var_type &type,
																					const char *name) {
	uint8_t count = 0;
	for (uint16_t i = 0; pgm_read_one_byte(&_var_info[i].vtype) != PARAM_NONE; i++) {
...
float Mav_Param::get_param_value_name_id(	enum var_type &type,
																					char *name,
																					uint8_t id) {
	uint8_t sum = 0;
	for (uint16_t i = 0; pgm_read_one_byte(&_var_info[i].vtype) != PARAM_NONE; i++) {
...
void Mav_Param::get_param_name_id(uint8_t id, char *name) {
	uint8_t sum = 0;
	for (uint16_t i = 0; pgm_read_one_byte(&_var_info[i].vtype) != PARAM_NONE; i++) {
		if (_var_info[i].vtype == PARAM_GROUP) {
			const struct GroupInfo *group_info =
