...
#define GOBJECT(v, name, class) { PARAM_GROUP, name,  &v, {group_info : class::var_info} }
...
class Mav_Param
{
	public:
		...
		Mav_Param(const struct Info *info)
		{
			...
			{
#ifdef DEBUG_ALL
				Serial.print("info name:"); Serial.println(info[i].name);
#endif
				...
				if (pgm_read_one_byte(&info[i].vtype) == PARAM_GROUP)
				{
					...
					{
#ifdef DEBUG_ALL
						Serial.print("G:");Serial.println(ginfo[a].name);
#endif
...
#ifdef DEBUG_ALL
			Serial.print("_num_vars:");Serial.print(_num_vars);
			Serial.print("_num_vars_all:");Serial.println(_num_vars_all);
#endif
...
template<typename NumericT, var_type Type, typename Class = Mav_Param>
class Mav_ParamT: public Mav_Param
{
	public:
		typedef void (*task_fn_t)(NumericT);

		Mav_ParamT() :
				_value(0), _interrupt(0), _changed(false)
		{
		}

		Mav_ParamT(task_fn_t task) :
				_value(0), _interrupt(task), _changed(false)
		{
		}

		Mav_ParamT(NumericT value, task_fn_t task) :
				_value(value), _interrupt(task), _changed(false)
		{
		}
		...
		template<typename T>
		Mav_ParamT<NumericT, Type, Class>& operator=(T val)
		{
			_value = val;
			return *this;
		}
		...
		Mav_ParamT<NumericT, Type, Class>& operator=(const NumericT &v)
		{
			_value = v;
			return *this;
		}
		...
		void attache_interrupt(task_fn_t interupt)
		{
			_interrupt = interupt;
		}
