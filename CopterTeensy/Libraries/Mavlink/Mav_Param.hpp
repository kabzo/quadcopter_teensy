...
public:
	...
	Mav_Param(const struct Info *info) {
		...
		for (i = 0; pgm_read_one_byte(&info[i].vtype) != PARAM_NONE; i++) {
			if (pgm_read_one_byte(&info[i].vtype) == PARAM_GROUP) {
				const struct GroupInfo *ginfo = (const struct GroupInfo *) PGM_POINTER(
						&info[i].group_info);
				uint16_t a;
	...
	static float get_param_value_id(uint8_t id, enum var_type &type);
	static void get_param_name_id(uint8_t id, char *name);
	static float get_param_value_id_name(	uint8_t &id,
																				enum var_type &type,
																				const char *name);
	static float get_param_value_name_id(	enum var_type &type,
																				char *name,
																				uint8_t id);
