...
class GCS_Mavlink {
	public:
		...
		void send_attitude(Attitude_t &attitude);
	...
	private:
		...
		uint32_t lastTimeMavlinkReceive;
		static const uint16_t readMavlink = 100;
...
void GCS_Mavlink::handle_param_request_read(mavlink_message_t *msg) {
	...
	if (param != NULL) {
		...
		mavlink_msg_param_value_pack(systemId, MAV_COMP_ID_ALL, msg, packet.param_id, value, vtype, param_count, packet.param_index);
		uint8_t buf[MAVLINK_MAX_PACKET_LEN];
		uint16_t len = mavlink_msg_to_send_buffer(buf, msg);
		Serial.write(buf, len);
	}
...
void GCS_Mavlink::handle_param_set(mavlink_message_t *msg) {
	...
	if (param != NULL) {
		...
		mavlink_msg_param_value_pack(systemId, MAV_COMP_ID_ALL, msg, packet.param_id, param->cast_to_float_mav(vtype), vtype, param_count, Mav_Param::get_param_id_name(packet.param_id));
		uint8_t buf[MAVLINK_MAX_PACKET_LEN];
		uint16_t len = mavlink_msg_to_send_buffer(buf, msg);
		Serial.write(buf, len);
	}
...
void GCS_Mavlink::send_hearthbeat(enum MAV_STATE mav_state_define,
		enum MAV_MODE mav_mode_define) {
	mavlink_message_t msg;
	uint8_t buf[MAVLINK_MAX_PACKET_LEN];			// Pack the message
	mavlink_msg_heartbeat_pack(systemId, 0, &msg, MAV_TYPE_QUADROTOR, MAV_AUTOPILOT_GENERIC, mav_mode_define, MAV_MODE_FLAG_SAFETY_ARMED, mav_state_define);
...
void GCS_Mavlink::send_hearthbeat() {
	mavlink_message_t msg;
	uint8_t buf[MAVLINK_MAX_PACKET_LEN];			// Pack the message
	mavlink_msg_heartbeat_pack(systemId, 0, &msg, MAV_TYPE_QUADROTOR, MAV_AUTOPILOT_GENERIC, mav_mode, MAV_MODE_FLAG_SAFETY_ARMED, mav_state);
...
void GCS_Mavlink::send_hearthbeat_1HZ(enum MAV_STATE mav_state_defined,
	...
	if ((millis() - lastTimesendHeathbeat) > sendHearthbeat1HZ) {
		mavlink_message_t msg;
		uint8_t buf[MAVLINK_MAX_PACKET_LEN];			// Pack the message
		mavlink_msg_heartbeat_pack(systemId, 0, &msg, MAV_TYPE_QUADROTOR, MAV_AUTOPILOT_GENERIC, mav_mode_defined, MAV_MODE_FLAG_SAFETY_ARMED, mav_state_defined);
...
void GCS_Mavlink::handle_manual_control(mavlink_message_t *msg) {
	if (mav_mode == MAV_MODE_MANUAL_ARMED) {
		...
		if (manual_control_t.target == systemId) {
			...
			param_container.manual_rcZ =
					map(manual_control_t.z, -680, 690, 1050, 1800);
			param_container.manual_rcR =
					map(manual_control_t.r, -730, 660, -180, 180);
...
//		Serial.println(param_container.manual_rcR);
		...
		param_container.manual_rcX = 0;
		param_container.manual_rcY = 0;
		param_container.manual_rcZ = 0;
		param_container.manual_rcR = 0;
	}

}

void GCS_Mavlink::send_attitude(Attitude_t &attitude) {
	if ((millis() - lastTimesendAttitude) > sendAttitudebeat10HZ) {
//		Serial.println(attitude.yaw);
		mavlink_message_t msg;
		uint8_t buf[MAVLINK_MAX_PACKET_LEN];
		mavlink_msg_attitude_pack(systemId, MAV_COMP_ID_IMU, &msg, millis(), attitude.roll
				* M_PI / 180, attitude.pitch * M_PI / 180, attitude.yaw * M_PI / 180, 0, 0, 0);
		uint16_t len = mavlink_msg_to_send_buffer(buf, &msg);
		Serial.write(buf, len);
		Serial.flush();
